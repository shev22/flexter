...
class AiringTodayRepository
{
    ...
    public function airingToday($mediaType,  $pages)
    {
        ...
        // $url = 'https://api.themoviedb.org/3/' . $mediaType . '/airing_today';
        // $nbPages = $pages;
        // // $media = array();
        ...
        // for ($i = 1; $i <= $pages; $i++) {
        //     $promises[] = Http::withToken(config('services.tmdb.token'))->async()->get('https://api.themoviedb.org/3/'.$mediaType.'/airing_today?page=' . $i);
        // }
        ...
        $time = time();
        $statistics = [
            'repository' => 'airingtoday-' . $mediaType,
            'quantity' => null,
            'duration' => null,
            'status' => null,
            'error_message' => null,
